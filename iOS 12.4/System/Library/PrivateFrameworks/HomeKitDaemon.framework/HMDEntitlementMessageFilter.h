/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, February 3, 2021 at 10:26:04 PM Australian Eastern Daylight Time
* Operating System: Version 12.4 (Build 16G77)
* Image Source: /System/Library/PrivateFrameworks/HomeKitDaemon.framework/HomeKitDaemon
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <HomeKitDaemon/HMDMessageFilter.h>
#import <libobjc.A.dylib/HMFLogging.h>

@class HMFMessageDestination, NSArray, NSString;

@interface HMDEntitlementMessageFilter : HMDMessageFilter <HMFLogging> {

	unsigned long long _entitlements;
	HMFMessageDestination* _target;
	NSArray* _messageNames;

}

@property (nonatomic,readonly) unsigned long long entitlements;              //@synthesize entitlements=_entitlements - In the implementation block
@property (nonatomic,readonly) HMFMessageDestination * target;               //@synthesize target=_target - In the implementation block
@property (nonatomic,copy,readonly) NSArray * messageNames;                  //@synthesize messageNames=_messageNames - In the implementation block
@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
+(id)logCategory;
-(unsigned long long)entitlements;
-(id)logIdentifier;
-(id)initWithEntitlements:(unsigned long long)arg1 target:(id)arg2 messageNames:(id)arg3 ;
-(BOOL)acceptMessage:(id)arg1 target:(id)arg2 errorReason:(id*)arg3 ;
-(BOOL)__shouldCheckMessage:(id)arg1 ;
-(NSArray *)messageNames;
-(HMFMessageDestination *)target;
-(id)initWithName:(id)arg1 ;
@end

