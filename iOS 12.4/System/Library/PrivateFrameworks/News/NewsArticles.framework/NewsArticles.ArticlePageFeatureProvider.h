/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, February 3, 2021 at 10:27:34 PM Australian Eastern Daylight Time
* Operating System: Version 12.4 (Build 16G77)
* Image Source: /System/Library/PrivateFrameworks/News/NewsArticles.framework/NewsArticles
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@interface NewsArticles.ArticlePageFeatureProvider : _UKNOWN_SUPERCLASS_ {

	 article;
	 issue;
	 articleAccessMap;
	 featureAvailability;
	 featureConfiguration;
	 coordinator;
	 textSizeManagerFactory;
	 reportConcernModule;
	 sharingActivityItemFactory;
	 menuBarItemHandlerProviderFactory;
	 windowToolbarItemHandlerProviderFactory;
	 issueCoverBarItemFactory;
	 tableOfContentsModule;
	 modalViewControllerFactory;
	 paidAccessChecker;
	 alertPresenter;
	 paywallTypeProvider;

}
@end

