/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, February 3, 2021 at 10:27:35 PM Australian Eastern Daylight Time
* Operating System: Version 12.4 (Build 16G77)
* Image Source: /System/Library/PrivateFrameworks/News/NewsFeed.framework/NewsFeed
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


#import <NewsFeed/NewsFeed-Structs.h>
@class TUShineImageRequest, NEPremiumBadgeMask;

@interface NEPremiumBadgeRequest : NSObject {

	 style;
	 shineImageRequest;
	 mask;

}

@property (readonly,nonatomic) long long style; 
@property (readonly,nonatomic) TUShineImageRequest * shineImageRequest; 
@property (readonly,nonatomic) NEPremiumBadgeMask * mask; 
-(TUShineImageRequest *)shineImageRequest;
-(id)initWithStyle:(long long)arg1 color:(id)arg2 bounds:(CGRect)arg3 mask:(id)arg4 ;
-(id)initWithStyle:(long long)arg1 color:(id)arg2 bounds:(CGRect)arg3 mask:(id)arg4 offset:(CGPoint)arg5 ;
-(id)init;
-(long long)style;
-(NEPremiumBadgeMask *)mask;
@end

